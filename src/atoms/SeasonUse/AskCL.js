...
const getFill = (i,target,toLocation ,val, defaultFill, location, nextLocation) => {
    //console.log(val?.who);
    if (i===toLocation && val?.who?.includes(target)) return "#E5651A"
    if(val?.who?.includes(target)) return "#ff0013"
    if(i===toLocation) return "#cbcb27"
...
const Circle = ({onClick, id, xy, fill, fillbg}) => {

        return [
            <circle key={'ckmap-circle-bg_' + id} id={'map-circle-bg_' + id} cx={xy[0]} cy={xy[1]} r="35.41325"
                    fill={fillbg}/>,
            <g className="circles" id={'map-circle-g_' + id} key={'ckmap-circle-g_' + id} onClick={onClick}>
...
const AskCL = ({ title, onSubmit}) => {
    const [ret, setRet] = useState({classID:0,location:24});
    const [submit, setSubmit] = useState(false);
    if (!onSubmit) onSubmit=x=>{console.log("AskCL",x)}
        ...
        <div id="askContainer" className={submit?"submittedAskContainer":null}>
            <div>
                ...
                <div>
                    ...
                    <svg id="mapAskContainer" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1115.51722 876.47781">
                        {paths.map((x, i) => {
                            const y = x.split(' ');
                        ...
                        {circles.map((x, i) => {
                            const y = x.split(' ');
                            return (
                                <Circle canGo={mapData[i]?.canGo} onClick={res=>setRet(prev=>{return {...prev,location:parseInt(res.target.id.split("_")[1])}})} id={`${i}`} key={`ck${i}`}
                                        fill={getFill(i,ret.classID ,ret.location ,mapData[i], y[2], location, nextLocation)} fillbg={getFillBg(mapData[i])}
                                        xy={[y[0], y[1]]}/>
                            );
                        })}
