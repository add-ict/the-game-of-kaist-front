...
const Circle = ({id,xyc,dataRef,data,classID,fillbg}) => {
    const onClick = () => {
    ...
    return [
        <circle key={'ckmap-circle-bg_' + id} id={'map-circle-bg_' + id} cx={xyc[0]} cy={xyc[1]} r="35.41325"
                fill={fillbg}/>,
        <g className="circles" id={'map-circle-g_' + id} key={'ckmap-circle-g_' + id} onClick={data?.["class"]?.[classID].map.canGo[id]?onClick:null}>
            <circle id={'map-circle_' + id} key={'ckmap-circle_' + id} cx={xyc[0]} cy={xyc[1]} r="26.55993"
                    fill={data?.["class"]?.[classID]?.map?.canGo[id]?"#f00":(xyc[2]==="1"?"#CAE266":"#73BF4B")}></circle>
            <text id={'map-circle-text_' + id} key={'ckmap-circle-text_' + id} x={xyc[0]} y={xyc[1]}>{id}</text>
        </g>
    ];
...
const Map = ({state,mapData,dataRef,data,t}) => {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < 5; i++) {
            console.log(data)
            who[data?.["class"]?.[i]?.map?.location] += i + " "
        }
        for (let i = 0; i < 40; i++)
            tooltips.current[i].setContent(
                ...
                (who[i]?who[i]+"<br/>":"")+
                (mapData?.[i]?.cause?mapData?.[i]?.cause?.[t]+"<br/>":"")+
    ...
    return (
        <svg id="mapContainer" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1115.51722 876.47781">
            ...
            {circles.map((x, i) => {
                const y = x.split(' ');
                return (
                    <Circle id={`${i}`} key={`ck${i}`} dataRef={dataRef} xyc={y} mapData={mapData} fillbg={whos[i]?"#00fff6":"#fff"}/>
                );
            })}
